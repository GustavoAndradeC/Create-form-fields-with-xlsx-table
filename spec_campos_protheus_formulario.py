...
def type_textarea(row):
    ...
    tag_textarea = '''
                    <div class="form-group  col-md-3 col-xs-12 col-sm-3"">
                      <label for="'''+campo+'''" id="'''+campo+'''_L">i18n.translate("'''+campo+'''")</label> 
...
def type_zoom(row):
    ...
    tag_zoom = '''
                    <div class='form-field col-xs-3 col-sm-3 col-md-3'>
                        <label for="'''+campo+'''" id="'''+campo+'''_L">i18n.translate("'''+campo+'''")'''+obrigatorio+'''</label>
                        <div class='input-group col-xs-12 col-sm-12 col-md-12'>
                            <input type="zoom" class='col-xs-12 col-sm-12 col-md-12' id="'''+campo+'''" name="'''+campo+'''"
                             data-zoom="'''+opcoes+'''"
                            />
                        </div>
                    </div>'''
...
def type_input(row):
    ...
    if tipo_cp:
        tag_text = '''
                        <div class='form-group  col-md-3 col-xs-12 col-sm-3'>
                            <label for="'''+ campo  +'''" id="'''+ campo  +'''_L">i18n.translate("'''+ campo  +'''")'''+ obrigatorio +'''</label> 
                            <input type="'''+ tipo_cp  +'''" name="'''+ campo  +'''" id="'''+ campo  +'''" class="form-control" maxlength="'''+ tamanho  +'''" '''+ ('readonly' if editavel == 'N' else '') + '''>
                        </div>
        '''
    return tag_text
...
def type_select(row):
    ...
    for idx,opt in enumerate(options_select_arr):
        ...
        options += '''    <option value="'''+ value[0] +'''">i18n.translate("'''+ tag_opt +'''")</option>
                        '''
...
#Read CSV with the data
data_frame = pd.read_csv("spec_campos_protheus_formulario.csv", sep=";",encoding='utf-8')
